...
class AppSize extends InheritedWidget {
  ...
  void init({BuildContext? context}) {
    ...
    double safeWidth = min((screenWidth - _safeAreaHorizontal), _maxMobileWidth);
  ...
  static AppSize of(BuildContext context) {
    final AppSize? result = context.dependOnInheritedWidgetOfExactType<AppSize>();
  ...
  bool updateShouldNotify(covariant AppSize oldWidget) {
    if (oldWidget.screenWidth != screenWidth || oldWidget.screenHeight != screenHeight) {
      return true;
    }
    return false;
  }
